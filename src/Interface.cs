...
//
// Hooks never fire from Record - serialization, deserialization, cloning, and checksumming all leave them silent, including the load-time fill of a component or singleton that the save predates. Having-fired is conceptually part of the recorded state, and migrating hook-derived state into an old save is the host's problem. An entity that arrived by load still gets its remove hooks when it goes.
//
// On add, per-component hooks fire first, then global hooks. On remove, global hooks fire first, then per-component hooks. Internal order is currently undefined. An exception thrown by a hook is reported and the remaining hooks still run.
//
...
// Fired on a singleton whenever any entity is added. Only singleton components may implement this.
public interface IOnAddGlobal
{
    public void OnAddGlobal(Entity entity);
}

// Fired on a singleton whenever any entity is removed. Only singleton components may implement this.
public interface IOnRemoveGlobal
{
